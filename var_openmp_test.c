 ...
 * -- fine-grained OpenMP (with private)
 * -- var calcs inline
 *
 * Requirements
...
int main(int argc, char** argv) {
  ...
  if (filePtr == NULL) {
	...
	double minabs = x[0];
	double maxabs = x[0];
#pragma omp parallel default(none) shared(n, x, mean, sum, squaredDiffs, minabs, maxabs)
{
	#pragma omp for schedule(guided) reduction(+:sum)
		...
		double val;
	#pragma omp for schedule(guided) private(val) nowait
		for (int i=0; i<n; i++) {
		  val = (x[i] - mean);
	...
	#pragma omp for schedule(guided) private(val) reduction(min:minabs) nowait
		for (int i=0; i<n; i++) {
		  val = fabs(x[i]);
		...
		////double maxabs = x[0];
	#pragma omp for schedule(guided) private(val) reduction(max:maxabs)
		for (int i=0; i<n; i++) {
		  val = fabs(x[i]);
		  maxabs = (val > maxabs) ? val : maxabs;		}
}
	// end of all parallel regions
